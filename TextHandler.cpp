...
void TextHandler::insert(std::string word, std::map<std::string, int>* lst) {
    if ( lst->find(word) == lst->end() ) {
        lst->insert(std::pair<std::string, int>(lastWord, 0));
    }
    lst->at(word) += 1;
}
...
void TextHandler::setInWord() {
    this->inWord = !this->inWord;
}

void TextHandler::parseText(std::ifstream& file) {
    int diff = 'a' - 'A';
    char symbol;
    ...
    for ( ; file.get(symbol) ; ) {
        ...
        if ( isSpecial(symbol) ) {
            ...
            if ( inWord ) {
                setInWord();
                insert(lastWord, words);
                insert(lastWord, wordsStatistics);
                wordsQuantity += 1;
                lastWord = "";
            }
        }
        if ( isDelimiter(symbol) && inWord ) {
            setInWord();
            insert(lastWord, words);
            insert(lastWord, wordsStatistics);
            wordsQuantity += 1;
